...
## 002.auth
- [✅] Add native browser auth state support (storageState) -> [002.auth.01-storage-state.md](docs/tasks/002.auth.01-storage-state.md)
...
- [◒] **Task 23: Multi-tab navigation** -> [017.features.01-multi-tab-navigation.md](docs/tasks/017.features.01-multi-tab-navigation.md)
  - `navigate` action: go to any URL mid-session
  - `newTab` action: open a new browser tab (optionally navigate)
  - `switchTab`/`closeTab`: manage multiple open tabs
  - Tab info injected into LLM prompt when multiple tabs are open
  - All tabs share cookies/session (same browser context)
  - Speech bubble re-injected on tab switch
  - Puppet changes: types, session handler, fluent API

## 018.features

- [✅] **Task 24: Adaptive focus screenshots** -> [018.features.01-adaptive-focus-screenshots.md](docs/tasks/018.features.01-adaptive-focus-screenshots.md)
  - Detect region of interest (ROI) when new elements appear (dropdowns, modals)
  - Take focused clip screenshot of ROI instead of full viewport
  - Annotate only ROI elements for less visual noise
  - Offset coordinate fallback clicks from clip-relative to viewport-relative
  - Enhanced element legend: in-view vs off-screen grouping
  - **Depends on:** Task 12 (Sight Mode), Task 20 (Portal Scanning), Puppet clip screenshots
